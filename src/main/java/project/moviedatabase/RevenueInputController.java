...
public class RevenueInputController implements Serializable
{
    ...
    public void onConfirmClick(ActionEvent actionEvent) throws IOException, ClassNotFoundException {
        ...
        {
            if(m.getRevenue() >= inputLow && m.getRunningTime() <= inputHigh)
            {
                result.add(m);
            }
        }
        if(result.isEmpty())
        {
            main.wrongSearchInputAlert("range of revenue");
        }
        else
        {
