...
public class BudgetInputController implements Serializable
{
    ...
    public void onConfirmClick(ActionEvent actionEvent) throws IOException, ClassNotFoundException {
        ...
        {
            if(m.getBudget() >= inputLow && m.getRunningTime() <= inputHigh)
            {
                result.add(m);
            }
        }
        if(result.isEmpty())
        {
            main.wrongSearchInputAlert("range of budget");
        }
        else
        {
