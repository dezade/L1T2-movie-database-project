...
public class ProfitInputController implements Serializable
{
    ...
    public void onConfirmClick(ActionEvent actionEvent) throws IOException, ClassNotFoundException {
        ...
        {
            if(m.getProfit() >= inputLow && m.getRunningTime() <= inputHigh)
            {
                result.add(m);
            }
        }
        if(result.isEmpty())
        {
            main.wrongSearchInputAlert("range of profit");
        }
        else
        {
